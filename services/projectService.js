...
async function getFavoriteNodes(projectId){
    ...
    return await Project.findOne({_id:projectId}, {favoriteNodes:1, _id: 0});
}

async function getFavoriteNode(projectId,twitterId){
    logger.info(`[getFavoriteNode] - ${path.basename(__filename)}`);
    return await Project.findOne({ _id: projectId, favoriteNodes: { $in: [twitterId] } });
}

async function addFavoriteNode(projectId,twitterId){
    logger.info(`[addFavoriteNode] - ${path.basename(__filename)}`);

    return await Project.updateOne( { _id: projectId, favoriteNodes: { $ne: twitterId } },    { $addToSet: { favoriteNodes: twitterId } });
...
async function removeFavoriteNodeFromFavoriteNodes(projectId,twitterId){
    logger.info(`[removeFavoriteNodeFromFavoriteNodes] - ${path.basename(__filename)}`);
    return await Project.updateOne({ _id: projectId },{ $pull: { favoriteNodes: twitterId } });
}

async function removeFavoriteNodeFromTimeRangesNetwork(projectId,twitterId){
    logger.info(`[removeFavoriteNodeFromTimeRangesNetwork] - ${path.basename(__filename)}`);
    const project = await Project.findOne({_id: projectId}).populate('timeRanges');
    if (!project) {
